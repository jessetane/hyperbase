...
class HyperbaseTransportTcp extends EventTarget {
  constructor (opts = {}) {
    super()
  ...
  connect (host, port) {
    var socket = tcp.connect(port, host)
    var peer = this.setupPeer(socket)
  ...
  setupPeer (socket) {
    ...
    peer.address = 'tcp|' + socket.remoteAddress + '|' + socket.remotePort
    peer.serialize = req => {
...
export default HyperbaseTransportTcp
